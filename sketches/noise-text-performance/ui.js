const UI = (() => {
  ...
  const SHORT_LABELS = {
    scrollPos: 'Scroll',  autoScroll: 'AutoScr', masterVol: 'Vol',     modAmount: 'Mod',
    visualJag: 'Jag',     lpfFreq: 'LPF',        distortion: 'Dist',   reverbWet: 'RvbWet',
    jitterAmt: 'Jitter',  stutterProb: 'Stutter',masterPitch: 'Pitch',
    ...
    delayWet: 'DelWet',   preserve: 'Preserve',  fbkHpf: 'FbkHPF',     fbkNoise: 'FbkNoi',
    fbkSine: 'FbkSin',    fbkSineHz: 'FbkSnHz',
    // Bank 3
  ...
  const FORMATTERS = {
    scrollPos:   (m) => (m * 100).toFixed(0) + '%',
    autoScroll:  (m) => (m >= 0 ? '+' : '') + m.toFixed(2),
    masterVol:   (m) => (m * 100).toFixed(0) + '%',
    modAmount:   (m) => (m * 100).toFixed(0) + '%',
    visualJag:   (m) => m.toFixed(0),
    lpfFreq:     (m) => m >= 1000 ? (m / 1000).toFixed(1) + 'k' : m.toFixed(0) + 'Hz',
    distortion:  (m) => m.toFixed(2),
    reverbWet:   (m) => m.toFixed(2),
    jitterAmt:   (m) => m.toFixed(2),
    stutterProb: (m) => m.toFixed(3),
    masterPitch: (m) => m.toFixed(2) + '×',
    vSpatial:    (m) => m.toFixed(2),
    vTimeSpd:    (m) => m.toFixed(3),
    vFlowSpd:    (m) => m.toFixed(3),
    lpfRes:      (m) => m.toFixed(1),
    delayTime:   (m) => (m * 1000).toFixed(0) + 'ms',
    delayFbk:    (m) => m.toFixed(2),
    reverbDecay: (m) => m.toFixed(1) + 's',
    jitterFreq:  (m) => m.toFixed(2),
    reverseProb: (m) => m.toFixed(3),
    stutterMax:  (m) => m.toFixed(2) + 's',
    delayWet:    (m) => m.toFixed(2),
